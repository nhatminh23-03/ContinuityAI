...
| OPEN-17 | The independence gate rejects truthful *negations*. Observed live under `AI_PROVIDER=hybrid`: the model wrote the strength "Incident Recovery: assisted participation with support, **not** independent evidence" — an accurate description of assisted-only coverage — and `find_independence_language` rejected it, because `INDEPENDENCE_MARKERS` is a flat substring list with no negation handling, so "not independent" is indistinguishable from "independent". The failure direction is safe: it falls back to the template. The cost is that the model cannot use the natural word for a gap, and the log records the rejection as an overstatement when the model did the opposite. `validate_candidate_narrative`'s docstring documents the mirror blind spot — oblique claims passing — but not this one | Person A | Post-MVP |
